...
contract LSP8LoogiesBasic is LSP8IdentifiableDigitalAsset {
    ...
    /**
     * @notice Get the token metadata for a specific token
     * @param tokenId The token ID to get metadata for
     * @return metadata The token metadata JSON string
     */
    function getTokenMetadata(bytes32 tokenId) public view returns (string memory metadata) {
        // Check if token exists
        require(tokenOwnerOf(tokenId) != address(0), "Token does not exist");
        
        // Generate token metadata
        bytes memory metadataJSON = generateTokenMetadata(tokenId);
    ...
    /**
     * @notice Get the SVG for a specific token
     * @param tokenId The token ID to get SVG for
     * @return svg The SVG string
     */
    function getSVG(bytes32 tokenId) public view returns (string memory svg) {
        // Check if token exists
        require(tokenOwnerOf(tokenId) != address(0), "Token does not exist");
        
        // Generate SVG
        return generateSVGofTokenById(tokenId);
    }
